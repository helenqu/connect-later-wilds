...
def _move_to_front(a, special_items):
    special = []
    nonspecial = []
    for x in a:
        if x in special_items: special.append(x)
        else: nonspecial.append(x)
...
def load(module, path, device=None, tries=2):
    ...
    # TODO MX: hack for swav
    # get rid of projection and prototype keys
    state = OrderedDict([(k, v) for k, v in state.items() if 'prototypes' not in k and 'projection' not in k])
    # new_state = OrderedDict({'.'.join(k.split('.')[1:]): v for k, v in state.items() if 'projection' not in k and 'prototypes' not in k})
    # new_state = OrderedDict({'0.' + k: v for k, v in new_state.items()})
...
def move_to(obj, device):
    ...
    elif isinstance(obj, MixedY):
        return MixedY(*[move_to(v, device) for v in obj])
...
def detach_and_clone(obj):
    if torch.is_tensor(obj):
        ...
        return MixedY(*[detach_and_clone(v) for v in obj])
    elif isinstance(obj, list):
